...
def constraint_equal(x):
    ...
    y[1]=(1-comp[0])*(x[idmap[(0,1,0)]]+x[idmap[(1,1,0)]]+x[idmap[(2,1,0)]])-comp[0]*(x[idmap[(0,1,1)]]+x[idmap[(1,1,1)]]+x[idmap[(2,1,1)]]) # the right amount of product 0 must arrive at sink 1
    return y
...
# Objective function to minimize the costs (for each product we have a specific)    
def obj_costs(x):
