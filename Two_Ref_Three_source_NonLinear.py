...
#source_avai=np.zeros(number_source*number_product)
#sink_dem=np.zeros(number_sink*number_product)
...
def constraint_equal(x):
    y=np.zeros(4)
    y[0]=(1-comp[0])*(x[idmap[(0,0,0)]]+x[idmap[(1,0,0)]]+x[idmap[(2,0,0)]])-comp[0]*(x[idmap[(0,0,1)]]+x[idmap[(1,0,1)]]+x[idmap[(2,0,1)]]) # the right amount of product 0 must arrive at sink 0
    y[1]=(1-comp[1])*(x[idmap[(0,0,1)]]+x[idmap[(1,0,1)]]+x[idmap[(2,0,1)]])-comp[1]*(x[idmap[(0,0,0)]]+x[idmap[(1,0,0)]]+x[idmap[(2,0,0)]]) # the right amount of product 1 must arrive at sink 0
    y[2]=(1-comp[0])*(x[idmap[(0,1,0)]]+x[idmap[(1,1,0)]]+x[idmap[(2,1,0)]])-comp[0]*(x[idmap[(0,1,1)]]+x[idmap[(1,1,1)]]+x[idmap[(2,1,1)]]) # the right amount of product 0 must arrive at sink 1
    y[3]=(1-comp[1])*(x[idmap[(0,1,1)]]+x[idmap[(1,1,1)]]+x[idmap[(2,1,1)]])-comp[1]*(x[idmap[(0,1,0)]]+x[idmap[(1,1,0)]]+x[idmap[(2,1,0)]]) # the right amount of product 0 must arrive at sink 1
    return y
...
# Objective 
def obj_max_flow(x):
    F = (x[idmap[(0,0,0)]]+x[idmap[(1,0,0)]]+x[idmap[(2,0,0)]])+\
...
options={'ftol':1e-1}
result=minimize(obj_max_flow, x0, method='SLSQP', bounds=bounds,constraints=cons,options=options)
x=result.x  
